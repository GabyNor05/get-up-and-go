...
export default function PermissionsScreen({ darkMode, onSkip, onContinue }: Props) {
  ...
  const handleContinue = async () => {
    try {
      ...
      if (uid) {
        await goerService.updatePermissions(uid, {
          location: locationOn,
          camera: cameraOn,
          pushNotifications: notifOn,
        });
      }
      ...
      console.error("Failed to save permissions to Goer profile:", error);
      router.push("/(tabs)/(home)");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: bg, paddingTop: 50, paddingBottom: 20 }}>
      {/* Main content */}
      <ScrollView
        style={{ flex: 1 }}
        ...
        <View style={{ gap: 14 }}>
          {permissions.map((p, i) => (
            ...
            >
              ...
              {/* Text */}
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 15, fontWeight: "700", color: text, marginBottom: 3 }}>
                  {p.title}
                </Text>
